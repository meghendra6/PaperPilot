...
exports.createEvidenceMatrix = createEvidenceMatrix;
exports.normalizeEvidenceMatrixCell = normalizeEvidenceMatrixCell;
exports.upsertEvidenceMatrixCells = upsertEvidenceMatrixCells;
exports.upsertEvidenceMatrixRow = upsertEvidenceMatrixRow;
exports.calculateEvidenceMatrixCoverage = calculateEvidenceMatrixCoverage;
...
function normalizeColumn(column) {
    const extractionQuestion = (column.extractionQuestion || column.question || "").trim();
    if (!extractionQuestion)
        throw new Error(`Column ${column.id} requires an extraction question.`);
    return { ...column, extractionQuestion, question: extractionQuestion, enumValues: column.enumValues ? [...column.enumValues] : undefined };
}
function createEvidenceMatrix(params) {
    ...
    for (const column of columns) {
        ...
        if (seen.has(column.id))
            throw new Error(`Duplicate column ${column.id}`);
        seen.add(column.id);
    ...
    return { schemaVersion: 2, id: params.id, title, name: title, columns, papers, cells: [], rows: [], createdAt: now, updatedAt: now };
}
        ...
        if (column.valueType === "number") {
            ...
            if (typeof value !== "boolean") {
                ...
                if (["true", "yes", "1"].includes(normalized))
                    ...
                    throw new Error(`${column.id} must be boolean`);
            }
        }
        else if (column.valueType === "list") {
            value = Array.isArray(value) ? value.map(String) : String(value).split(/[,;]\s*/).filter(Boolean);
        }
        else {
            value = String(value);
...
function upsertEvidenceMatrixCells(matrix, incoming, now = new Date().toISOString()) {
    ...
    const byKey = new Map(matrix.cells.map((cell) => [`${cell.paperKey}\u0000${cell.columnId}`, cell]));
    for (const cell of incoming) {
        if (!paperKeys.has(cell.paperKey))
            throw new Error(`Unknown paper ${cell.paperKey}`);
        const column = columns.get(cell.columnId);
        if (!column)
            throw new Error(`Unknown column ${cell.columnId}`);
    ...
    const cells = [...byKey.values()].sort((left, right) => {
        const paperOrder = matrix.papers.findIndex((paper) => paper.paperKey === left.paperKey) - matrix.papers.findIndex((paper) => paper.paperKey === right.paperKey);
        return paperOrder || matrix.columns.findIndex((column) => column.id === left.columnId) - matrix.columns.findIndex((column) => column.id === right.columnId);
    });
    return { ...matrix, cells, rows: rebuildRows(matrix, cells, now), updatedAt: now };
}
function upsertEvidenceMatrixRow(matrix, row) {
    let papers = matrix.papers;
    if (!papers.some((paper) => paper.paperKey === row.paperKey)) {
        papers = [...papers, { paperKey: row.paperKey, title: row.title, attachmentKeys: row.attachmentKey ? [row.attachmentKey] : [] }];
    }
    return upsertEvidenceMatrixCells({ ...matrix, papers }, row.cells.map((cell) => ({ ...cell, paperKey: row.paperKey })));
}
function calculateEvidenceMatrixCoverage(matrix) {
    const cellCount = matrix.papers.length * matrix.columns.length;
    const filled = matrix.cells.filter((cell) => cell.status === "extracted" && cell.value !== null && !!cell.displayValue.trim());
    const evidenced = filled.filter((cell) => cell.evidence.length > 0);
    const columnMap = new Map(matrix.columns.map((column) => [column.id, column]));
    const requiredColumnCount = matrix.columns.filter((column) => column.requiredEvidence).length;
    const requiredCellCount = matrix.papers.length * requiredColumnCount;
    const requiredEvidenced = filled.filter((cell) => columnMap.get(cell.columnId)?.requiredEvidence && cell.evidence.length > 0);
    ...
    return {
        ...
        requiredEvidenceCoverage: requiredCellCount ? requiredEvidenced.length / requiredCellCount : 1,
    };
}
