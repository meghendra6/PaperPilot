...
exports.buildHybridIndex = buildHybridIndex;
exports.chunkTextForHybridIndex = chunkTextForHybridIndex;
exports.chunkPaperDocument = chunkPaperDocument;
...
function buildHybridIndex(params) {
    const dimensions = Math.max(32, Math.min(1024, Math.floor(params.embeddingDimensions ?? 192)));
    const chunks = params.chunks
        .filter((chunk) => chunk && typeof chunk.id === "string" && typeof chunk.text === "string" && chunk.text.trim())
        .map((chunk) => {
        const tokens = (0, tokenizer_1.tokenizeHybridOccurrences)(`${chunk.title || ""}\n${chunk.text}`, true);
        const termFrequency = {};
        ...
        return {
            ...
            tokens,
            termFrequency,
            embedding: (0, hashEmbedding_1.createHashEmbedding)(`${chunk.title || ""}\n${chunk.text}`, dimensions),
...
function chunkTextForHybridIndex(text, options = {}) {
    ...
    const overlap = Math.max(0, Math.min(chunkSize - 1, Math.floor(options.overlap ?? 220)));
    ...
    while (offset < normalized.length) {
        ...
        if (end < normalized.length) {
            ...
            if (boundary > offset + chunkSize * 0.55)
                end = boundary + 1;
        }
        const value = normalized.slice(offset, end).trim();
        if (value)
            result.push({ id: `chunk-${index}`, text: value, attachmentKey: options.attachmentKey });
        if (end >= normalized.length)
            break;
