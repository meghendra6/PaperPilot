...
exports.parseCrossPaperQuestionResponse = parseCrossPaperQuestionResponse;
exports.parseCrossPaperGradeResponse = parseCrossPaperGradeResponse;
const json_1 = __require("src/modules/comprehensionCheck/v2/json.ts");
const types_1 = __require("src/modules/evidence/types.ts");
const MODES = new Set(["compare", "synthesis", "conflict", "transfer", "timeline"]);
function strings(value) {
    return Array.isArray(value) ? value.filter((entry) => typeof entry === "string" && !!entry.trim()).map((entry) => entry.trim()) : [];
}
function uniqueStrings(value) { return [...new Set(strings(value))]; }
...
function parseCrossPaperQuestionResponse(params) {
    ...
    if (paperKeys.length < 2)
        throw new Error("A cross-paper question must reference at least two papers.");
    const allowedPaperKeys = params.allowedPaperKeys ?? new Set(params.concept?.paperKeys ?? paperKeys);
    for (const paperKey of paperKeys)
        if (!allowedPaperKeys.has(paperKey))
            throw new Error(`Unknown paper ${paperKey}`);
    const attachmentKeys = params.allowedAttachmentKeys ?? new Set(params.allowedAttachments ?? []);
    const rawRubric = Array.isArray(root.rubric) ? root.rubric : Array.isArray(root.criteria) ? root.criteria : [];
    if (!rawRubric.length)
    ...
    const rubric = rawRubric.map((entry, index) => {
        ...
        if (seen.has(id))
            throw new Error(`Duplicate rubric criterion ${id}`);
        seen.add(id);
        const requiredPaperKeys = uniqueStrings(object.requiredPaperKeys ?? object.paperKeys);
        if (!requiredPaperKeys.length)
            throw new Error(`Rubric criterion ${id} must reference papers.`);
        requiredPaperKeys.forEach((key) => { if (!allowedPaperKeys.has(key))
            throw new Error(`Unknown rubric paper ${key}`); });
        const expectedClaims = strings(object.expectedClaims ?? object.requiredClaims);
        ...
        return {
            ...
            requiredPaperKeys,
            expectedClaims,
            evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, { allowedAttachmentKeys: attachmentKeys.size ? attachmentKeys : undefined }),
            paperKeys: requiredPaperKeys,
            requiredClaims: expectedClaims,
        };
    });
    const evidence = {};
    if (root.evidence && typeof root.evidence === "object" && !Array.isArray(root.evidence)) {
        const rawEvidence = root.evidence;
        for (const paperKey of paperKeys)
            evidence[paperKey] = (0, types_1.normalizeEvidenceReferences)(rawEvidence[paperKey], { allowedAttachmentKeys: attachmentKeys.size ? attachmentKeys : undefined });
    }
    const now = params.now ?? new Date().toISOString();
    return {
        id: params.id ?? text(root.id, `cross-question-${now.replace(/[^0-9]/g, "")}`),
        conceptId: params.concept?.id ?? text(root.conceptId, "general"),
        mode,
        prompt: text(root.prompt, "Compare the selected papers."),
        paperKeys,
        difficulty: root.difficulty === "intermediate" ? "intermediate" : "advanced",
        createdAt: now,
        rubric,
        criteria: rubric,
        evidence,
...
function parseCrossPaperGradeResponse(params) {
    const root = (0, json_1.extractLastJsonObject)(params.response);
    const rawGrades = Array.isArray(root.grades) ? root.grades : Array.isArray(root.criterionScores) ? root.criterionScores : null;
    if (!rawGrades)
        throw new Error("Cross-paper grade must include criterion scores.");
    const allowedAttachmentKeys = params.allowedAttachmentKeys ?? new Set(params.allowedAttachments ?? []);
    const rubricById = new Map(params.question.rubric.map((criterion) => [criterion.id, criterion]));
    const seen = new Set();
    ...
    const grades = params.question.rubric.map((criterion) => {
        ...
        return {
            ...
            evidence: (0, types_1.normalizeEvidenceReferences)(object?.evidence, { allowedAttachmentKeys: allowedAttachmentKeys.size ? allowedAttachmentKeys : undefined }),
        };
    });
    const now = params.now ?? new Date().toISOString();
    return {
        id: params.id ?? text(root.id, `cross-attempt-${now.replace(/[^0-9]/g, "")}`),
        questionId: params.question.id,
        answer: params.answer ?? text(root.answer),
        learnerConfidence: Math.max(0, Math.min(1, Number(params.learnerConfidence ?? root.learnerConfidence) || 0)),
        grades,
        misconceptions: strings(root.misconceptions),
        graderConfidence: Math.max(0, Math.min(1, Number(root.graderConfidence) || 0)),
        createdAt: now,
    };
}
