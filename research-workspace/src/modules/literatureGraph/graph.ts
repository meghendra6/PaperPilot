...
exports.graphIntegrityIssues = exports.findLiteratureGraphPath = exports.findShortestLiteraturePath = void 0;
exports.createLiteratureGraph = createLiteratureGraph;
exports.addLiteratureNode = addLiteratureNode;
exports.addLiteratureEdge = addLiteratureEdge;
exports.mergeLiteratureGraph = mergeLiteratureGraph;
exports.validateLiteratureGraph = validateLiteratureGraph;
exports.shortestLiteraturePath = shortestLiteraturePath;
exports.connectedComponents = connectedComponents;
exports.literatureGraphIntegrityIssues = literatureGraphIntegrityIssues;
const NODE_KINDS = new Set(["paper", "concept", "claim", "method", "dataset"]);
const EDGE_KINDS = new Set(["introduces", "uses", "extends", "improves", "challenges", "supports", "contradicts", "compares", "evaluates_on", "related"]);
function createLiteratureGraph(paramsOrId, legacyName, legacyNow) {
    const params = typeof paramsOrId === "string"
        ? { id: paramsOrId, title: legacyName || "Literature Graph", now: legacyNow }
        : paramsOrId;
    const now = params.now ?? new Date().toISOString();
    const title = (params.title || params.name || "Literature Graph").trim();
    return { schemaVersion: 2, id: params.id, title, name: title, nodes: [], edges: [], createdAt: now, updatedAt: now };
}
function normalizeNode(node) {
    ...
    if (!id || !label)
        throw new Error("Graph nodes require id and label.");
...
function normalizeEdge(edge) {
    ...
    return { ...edge, id, source, target, kind, type: kind, confidence: Math.max(0, Math.min(1, Number(edge.confidence) || 0)), evidence: [...(edge.evidence || [])], verified: edge.verified === true };
}
function addLiteratureNode(graph, rawNode, now = new Date().toISOString()) {
    ...
    const nodes = graph.nodes.some((entry) => entry.id === node.id)
        ? graph.nodes.map((entry) => entry.id === node.id ? { ...entry, ...node } : entry)
        : [...graph.nodes, node];
...
function addLiteratureEdge(graph, rawEdge, now = new Date().toISOString()) {
    ...
    return { ...graph, edges: [...graph.edges.filter((entry) => entry.id !== edge.id), edge], updatedAt: now };
}
...
function validateLiteratureGraph(graph) {
    ...
    for (const edge of graph.edges) {
        if (edgeIds.has(edge.id))
            errors.push(`Duplicate edge ${edge.id}`);
        edgeIds.add(edge.id);
        ...
        if (edge.source === edge.target)
            errors.push(`Self edge ${edge.id}`);
        if (!edge.evidence.length)
            warnings.push(`Edge ${edge.id} has no evidence`);
        if (!edge.verified)
            warnings.push(`Edge ${edge.id} is unverified`);
    }
...
exports.findShortestLiteraturePath = shortestLiteraturePath;
...
exports.findLiteratureGraphPath = shortestLiteraturePath;
exports.graphIntegrityIssues = literatureGraphIntegrityIssues;
