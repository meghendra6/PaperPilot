...
exports.parseLiteratureGraphResponse = parseLiteratureGraphResponse;
...
function parseLiteratureGraphResponse(params) {
    ...
    const nodes = (0, json_1.readArray)(root.nodes, "nodes").map((entry, index) => {
        ...
        const paperKey = typeof object.paperKey === "string" ? object.paperKey.trim() : undefined;
        if (kind === "paper" && !paperKey)
            throw new Error(`Paper node ${String(object.id ?? "").trim()} requires a paperKey`);
        ...
        nodeIds.add(id);
        return {
            id, kind, type: kind, label: String(object.label ?? "").trim(),
            ...(paperKey ? { paperKey } : {}),
            ...(object.metadata && typeof object.metadata === "object" && !Array.isArray(object.metadata) ? { metadata: object.metadata } : {}),
        };
    });
    const edgeIds = new Set();
    const edges = (0, json_1.readArray)(root.edges, "edges").map((entry, index) => {
        ...
        edgeIds.add(id);
        return {
            id, source: String(object.source ?? "").trim(), target: String(object.target ?? "").trim(),
            kind, type: kind,
            ...(typeof object.label === "string" && object.label.trim() ? { label: object.label.trim() } : {}),
            confidence: Number(object.confidence) || 0,
            evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, { allowedAttachmentKeys: params.allowedAttachmentKeys }),
            verified: object.verified === true,
        };
    });
    const graph = (0, graph_1.mergeLiteratureGraph)({ graph: (0, graph_1.createLiteratureGraph)({ id: params.id, title: params.title || params.name, now: params.now }), nodes, edges, now: params.now });
