...
exports.parseCitationStanceResponse = parseCitationStanceResponse;
const json_1 = __require("src/modules/comprehensionCheck/v2/json.ts");
const types_1 = __require("src/modules/evidence/types.ts");
const STANCES = new Set(["supporting", "contrasting", "mentioning", "methodological", "unclear"]);
function parse(response, contexts, allowedAttachments) {
    ...
    for (const context of contexts) {
        if (!context?.id || contextIds.has(context.id))
            throw new Error(`Missing or duplicate citation context ${context?.id ?? ""}`);
        contextIds.add(context.id);
    }
    const allowed = new Map(contexts.map((context) => [context.id, context]));
    const allowedAttachmentKeys = new Set(allowedAttachments ?? []);
    const seen = new Set();
    const raw = Array.isArray(root.results) ? root.results : Array.isArray(root.contexts) ? root.contexts : [];
    const results = raw.map((entry, index) => {
        ...
        if (!source)
            throw new Error(`Unknown citation context ${contextId}`);
        ...
        return {
            contextId,
            ...(typeof object.targetClaimId === "string" && object.targetClaimId.trim() ? { targetClaimId: object.targetClaimId.trim() } : source.targetClaimId ? { targetClaimId: source.targetClaimId } : {}),
            stance,
            ...
            ...(typeof object.claim === "string" && object.claim.trim() ? { claim: object.claim.trim() } : {}),
            limitations: Array.isArray(object.limitations) ? object.limitations.filter((value) => typeof value === "string" && !!value.trim()).map((value) => value.trim()) : [],
            evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, { allowedAttachmentKeys }),
        };
    });
    return contexts.map((context) => results.find((result) => result.contextId === context.id) ?? {
        contextId: context.id,
        ...(context.targetClaimId ? { targetClaimId: context.targetClaimId } : {}),
        stance: "unclear",
        confidence: 0,
...
function parseCitationStanceResponse(responseOrParams, contexts) {
    return typeof responseOrParams === "string"
        ...
        : parse(responseOrParams.response, responseOrParams.contexts, responseOrParams.allowedAttachments);
}
