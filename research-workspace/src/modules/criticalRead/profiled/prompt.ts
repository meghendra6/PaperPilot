...
exports.buildProfiledCriticalReadPrompt = buildProfiledCriticalReadPrompt;
...
function buildProfiledCriticalReadPrompt(params) {
    const checks = params.profile.checks.map((entry) => `- ${entry.id}: ${entry.question}\n  Guidance: ${entry.guidance.join("; ")}`).join("\n");
