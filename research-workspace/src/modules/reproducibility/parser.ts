...
exports.parseReproducibilityAuditResponse = void 0;
exports.parseReproducibilityResponse = parseReproducibilityResponse;
const json_1 = __require("src/modules/comprehensionCheck/v2/json.ts");
const types_1 = __require("src/modules/evidence/types.ts");
const KINDS = new Set([
    "code", "commit", "dataset", "data", "model", "environment", "hardware",
    "training_config", "training", "inference_config", "inference",
    "evaluation_command", "evaluation", "random_seeds", "license", "results", "other",
]);
const AVAILABILITY = new Set([
    "available", "partial", "missing", "not_applicable", "unclear",
]);
...
function parseReproducibilityResponse(params) {
    ...
    const artifacts = (0, json_1.readArray)(root.artifacts, "artifacts").map((entry, index) => {
        const object = (0, json_1.readObject)(entry, `artifacts[${index}]`);
        const kind = normalizedKind(object.kind ?? object.category);
        const availability = normalizedAvailability(object.availability ?? object.status);
        ...
        return {
            ...
            status: availability,
            blocker,
            ...((0, json_1.readOptionalString)(object.value) ? { value: (0, json_1.readOptionalString)(object.value) } : {}),
            ...((0, json_1.readOptionalString)(object.url) ? { url: (0, json_1.readOptionalString)(object.url) } : {}),
            ...((0, json_1.readOptionalString)(object.version) ? { version: (0, json_1.readOptionalString)(object.version) } : {}),
            ...((0, json_1.readOptionalString)(object.notes) ? { notes: (0, json_1.readOptionalString)(object.notes) } : {}),
            evidence,
            confidence: clamp01(object.confidence ?? (evidence.length ? 0.7 : 0.35)),
        };
    });
    const steps = (Array.isArray(root.steps) ? root.steps : []).map((entry, index) => {
    ...
    const blockers = (0, json_1.readArray)(root.blockers, "blockers").map((entry, index) => {
        if (typeof entry === "string") {
            return { id: `blocker-${index + 1}`, severity: "major", description: entry.trim(), mitigation: "Resolve or document this blocker before claiming reproduction.", evidence: [] };
        }
        ...
        return {
            ...
            severity,
            description: text(object.description, text(object.title, "Unspecified blocker")),
            mitigation: text(object.mitigation, "Clarify the missing information or use a documented substitute."),
            evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, { allowedAttachmentKeys }),
        };
    });
    for (const artifact of artifacts) {
        if (!artifact.blocker)
            continue;
        if (blockers.some((blocker) => blocker.description.toLowerCase().includes(artifact.label.toLowerCase())))
            continue;
        blockers.push({
            ...
            mitigation: artifact.notes || `Provide or document ${artifact.label.toLowerCase()}.`,
            evidence: artifact.evidence,
        });
    }
    const minimumViableReproduction = stringArray(root.minimumViableReproduction ?? root.minimalReproductionSteps);
    const verificationChecks = stringArray(root.verificationChecks ?? root.verificationCommands);
    const effortText = text(root.estimatedEffort, "unknown");
    const estimatedEffort = effortText === "low" || effortText === "medium" || effortText === "high" ? effortText : "unknown";
    return {
        schemaVersion: 2,
...
exports.parseReproducibilityAuditResponse = parseReproducibilityResponse;
