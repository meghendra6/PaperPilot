...
exports.buildClaimExtractionPrompt = buildClaimExtractionPrompt;
exports.parseClaimExtractionResponse = parseClaimExtractionResponse;
const claimLedger_1 = __require("src/modules/evidence/claimLedger.ts");
const types_1 = __require("src/modules/evidence/types.ts");
const json_1 = __require("src/modules/comprehensionCheck/v2/json.ts");
const VERIFICATION_STATUSES = new Set(["verified", "partially_verified", "unverified", "conflicting"]);
...
function parseClaimExtractionResponse(params) {
    ...
    let ledger = (0, claimLedger_1.createClaimLedger)(params.paperKey ?? params.attachmentKey, params.now);
    for (const [index, entry] of (0, json_1.readArray)(root.claims, "claims").entries()) {
        ...
        const contradictions = (0, types_1.normalizeEvidenceReferences)(claim.contradictions, {
            allowedAttachmentKeys: allowed,
        });
        const inferredStatus = (0, claimLedger_1.inferClaimVerificationStatus)(support, contradictions);
        const verificationStatus = String(claim.verificationStatus || inferredStatus);
        if (!VERIFICATION_STATUSES.has(verificationStatus))
            throw new Error(`Unsupported claim verification status: ${verificationStatus}`);
