...
exports.toLearnerQuestionView = toLearnerQuestionView;
exports.toLearnerAttemptFeedback = toLearnerAttemptFeedback;
exports.toMasteryDashboardView = toMasteryDashboardView;
...
function evidenceLinks(references) {
    return references.map((reference) => ({
        ...
        reference: {
            ...reference,
            ...(reference.sectionPath ? { sectionPath: [...reference.sectionPath] } : {}),
...
function toLearnerAttemptFeedback(attempt) {
    const rubricById = new Map(attempt.question.rubric.map((criterion) => [criterion.id, criterion]));
    return {
        ...
        criteria: attempt.grade.criterionGrades.map((grade) => ({
            ...
            description: rubricById.get(grade.criterionId)?.description ?? "Unknown rubric criterion",
            score: grade.score,
...
function toMasteryDashboardView(session) {
    ...
    return {
        phase: session.phase,
        testedConcepts: Object.values(session.conceptStates).filter((state) => state.attemptCount > 0).length,
        totalConcepts: session.blueprint.concepts.length,
