...
exports.createMasterySession = createMasterySession;
exports.selectNextConcept = selectNextConcept;
exports.setPendingQuestion = setPendingQuestion;
exports.applyMasteryGrade = applyMasteryGrade;
exports.resolveMisconception = resolveMisconception;
exports.calculateMasteryMetrics = calculateMasteryMetrics;
exports.calculateCompletionStatus = calculateCompletionStatus;
const reviewScheduler_1 = __require("src/modules/comprehensionCheck/v2/reviewScheduler.ts");
const types_1 = __require("src/modules/comprehensionCheck/v2/types.ts");
function iso(clock) {
...
function prerequisitesReady(session, concept) {
    return concept.prerequisites.every((id) => session.conceptStates[id]?.status === "mastered");
}
function openMajorMisconceptionCount(session, conceptId) {
    return session.misconceptions.filter((entry) => entry.status === "open" &&
        entry.severity === "major" &&
        (conceptId === undefined || entry.conceptId === conceptId)).length;
}
/**
...
function selectNextConcept(session, now = new Date(session.updatedAt)) {
    ...
    const indexById = new Map(session.blueprint.concepts.map((concept, index) => [concept.id, index]));
    const scored = session.blueprint.concepts
        ...
        if (!state)
            throw new Error(`Missing concept state for ${concept.id}.`);
        let priority = 0;
...
function setPendingQuestion(session, question, clock) {
    ...
    const concept = session.blueprint.concepts.find((entry) => entry.id === question.conceptId);
    if (!concept || !session.conceptStates[question.conceptId]) {
        throw new Error(`Unknown question concept: ${question.conceptId}`);
    }
    const expectedRubric = new Map(concept.rubric.map((criterion) => [criterion.id, criterion]));
    if (question.rubric.length !== concept.rubric.length) {
        throw new Error("Question rubric does not match its validated concept rubric.");
    }
    for (const criterion of question.rubric) {
        const expected = expectedRubric.get(criterion.id);
        if (!expected || expected.maxScore !== criterion.maxScore || expected.essential !== criterion.essential) {
            throw new Error(`Question rubric criterion ${criterion.id} does not match the blueprint.`);
        }
    }
...
function normalizeGrade(question, grade) {
    ...
    for (const criterionGrade of grade.criterionGrades) {
        if (gradesById.has(criterionGrade.criterionId)) {
            throw new Error(`Duplicate criterion grade: ${criterionGrade.criterionId}.`);
        }
    ...
    for (const rubric of question.rubric) {
        ...
        earned += score;
        possible += maxScore;
        if (rubric.essential && score < maxScore * 0.5)
            essentialPassed = false;
        gradesById.delete(rubric.id);
    }
    if (gradesById.size > 0) {
        throw new Error(`Grade references unknown criterion ${gradesById.keys().next().value}.`);
    }
    const normalized = possible > 0 ? earned / possible : 0;
    return { score: clamp01(normalized), passed: normalized >= 0.7 && essentialPassed };
}
function successfulDelayedReviews(session, conceptId) {
    return session.attempts.filter((attempt) => attempt.question.conceptId === conceptId &&
        attempt.delayedReview &&
        attempt.normalizedScore >= 0.8).length;
}
function applyMasteryGrade(input) {
    ...
    const learnerConfidence = input.learnerConfidence === undefined ? undefined : clamp01(input.learnerConfidence);
    const hintLevel = Math.max(0, Math.min(5, Math.floor(input.hintLevel ?? 0)));
    const majorMisconception = input.grade.misconceptions.some((entry) => entry.severity === "major");
    ...
    const previousMisconceptions = input.session.misconceptions.map((entry) => {
        if (entry.conceptId !== question.conceptId || !normalized.passed || majorMisconception)
            return entry;
    ...
    const openMajorAfterGrade = [...previousMisconceptions, ...newMisconceptions].some((entry) => entry.conceptId === question.conceptId && entry.status === "open" && entry.severity === "major");
    const status = bestScore >= 0.8 && !openMajorAfterGrade ? "mastered" : "developing";
    const nextReviewAt = (0, reviewScheduler_1.scheduleNextReview)({
        submittedAt: now,
        normalizedScore: normalized.score,
        hintLevel,
        previousSuccessfulReviews: successfulDelayedReviews(input.session, question.conceptId),
        majorMisconception,
    });
    let next = {
        ...input.session,
...
function resolveMisconception(session, misconceptionId, status, clock) {
    ...
    const misconceptions = session.misconceptions.map((entry) => {
        if (entry.id !== misconceptionId)
            return entry;
        found = true;
        const validTransition = (entry.status === "open" && status === "repaired") ||
            (entry.status === "repaired" && status === "retested");
        if (!validTransition)
            throw new Error(`Invalid misconception transition: ${entry.status} -> ${status}`);
        return {
            ...entry,
            status,
    ...
    if (!found)
        throw new Error(`Unknown misconception: ${misconceptionId}`);
    return { ...session, misconceptions, updatedAt: now };
}
function calculateMasteryMetrics(session) {
    const conceptsById = new Map(session.blueprint.concepts.map((concept) => [concept.id, concept]));
    const totalWeight = session.blueprint.concepts.reduce((sum, concept) => sum + conceptWeight(concept), 0);
    const testedWeight = session.blueprint.concepts.reduce((sum, concept) => {
        return sum + (session.conceptStates[concept.id]?.attemptCount > 0 ? conceptWeight(concept) : 0);
    }, 0);
    let earned = 0;
    let possible = 0;
    ...
    for (const attempt of session.attempts) {
        ...
        if (attempt.learnerConfidence !== undefined) {
            calibrationErrors.push(Math.abs(attempt.learnerConfidence - attempt.normalizedScore));
        }
    ...
    return {
        ...
        calibration: calibrationErrors.length > 0
            ? 1 - calibrationErrors.reduce((sum, value) => sum + value, 0) / calibrationErrors.length
            : null,
        retention: retentionPossible > 0 ? retentionEarned / retentionPossible : null,
    };
}
function calculateCompletionStatus(session) {
    const metrics = calculateMasteryMetrics(session);
    const coreConcepts = session.blueprint.concepts.filter((concept) => concept.importance === "core");
    const coreMastered = coreConcepts.filter((concept) => session.conceptStates[concept.id]?.status === "mastered").length;
    const coreMasteryRatio = coreConcepts.length > 0 ? coreMastered / coreConcepts.length : 1;
    const dimensions = new Set(session.blueprint.concepts.map((concept) => concept.dimension));
    const coveredDimensions = new Set(session.attempts.map((attempt) => session.blueprint.concepts.find((concept) => concept.id === attempt.question.conceptId)
        ?.dimension));
    const allPresentDimensionsCovered = [...dimensions].every((dimension) => coveredDimensions.has(dimension));
    const advancedOrTransferPassed = session.attempts.some((attempt) => attempt.passed &&
        (attempt.question.difficulty === "advanced" || attempt.question.mode === "transfer"));
    ...
    if (openMajor > 0)
        reasons.push("A major misconception is still open.");
    return {
