...
exports.parseMasteryBlueprintResponse = parseMasteryBlueprintResponse;
exports.parseMasteryQuestionResponse = parseMasteryQuestionResponse;
exports.parseMasteryGradeResponse = parseMasteryGradeResponse;
...
function expectedClaim(value, index, normalization) {
    ...
    return {
        ...
        evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, normalization),
    };
}
function rubricCriterion(value, index, normalization) {
    ...
    return {
        ...
        description: (0, json_1.readString)(object.description, `rubric[${index}].description`),
        maxScore: (0, json_1.readNumber)(object.maxScore, `rubric[${index}].maxScore`, { min: 0.1 }),
...
function masteryConcept(value, index, normalization) {
    ...
    const expectedClaims = (0, json_1.readArray)(object.expectedClaims ?? [], `concepts[${index}].expectedClaims`).map((entry, claimIndex) => expectedClaim(entry, claimIndex, normalization));
    const rubric = (0, json_1.readArray)(object.rubric ?? [], `concepts[${index}].rubric`).map((entry, criterionIndex) => rubricCriterion(entry, criterionIndex, normalization));
    if (rubric.length === 0) {
        throw new Error(`concepts[${index}].rubric must contain at least one criterion.`);
    }
    ...
    return {
        ...
        dimension: enumValue(object.dimension, `concepts[${index}].dimension`, DIMENSIONS),
        importance: enumValue(object.importance, `concepts[${index}].importance`, IMPORTANCE),
        learningObjective: (0, json_1.readString)(object.learningObjective, `concepts[${index}].learningObjective`),
...
function assertAcyclic(concepts) {
    ...
    const visit = (id, path) => {
        ...
        if (visiting.has(id)) {
            ...
            throw new Error(`Mastery blueprint contains a prerequisite cycle: ${cycle}`);
        }
        visiting.add(id);
...
function validateBlueprint(blueprint, options) {
    ...
    if (options.maxConcepts !== undefined && blueprint.concepts.length > options.maxConcepts) {
        throw new Error(`Mastery blueprint has ${blueprint.concepts.length} concepts; maximum is ${options.maxConcepts}.`);
    }
    ...
    for (const concept of blueprint.concepts) {
        for (const prerequisite of concept.prerequisites) {
            if (!ids.has(prerequisite)) {
                throw new Error(`Concept ${concept.id} references unknown prerequisite ${prerequisite}.`);
            }
    ...
    if (options.requireAllDimensions) {
        const present = new Set(blueprint.concepts.map((concept) => concept.dimension));
        const missing = DIMENSION_VALUES.filter((dimension) => !present.has(dimension));
        if (missing.length > 0) {
            throw new Error(`Mastery blueprint is missing dimensions: ${missing.join(", ")}.`);
        }
    }
    return blueprint;
}
function parseMasteryBlueprintResponse(text, options = {}) {
    ...
    const blueprintObject = (0, json_1.readObject)(root.blueprint ?? root, "blueprint");
    const normalization = evidenceOptions(options);
    const concepts = (0, json_1.readArray)(blueprintObject.concepts, "blueprint.concepts").map((entry, index) => masteryConcept(entry, index, normalization));
...
function parseMasteryQuestionResponse(text, fallback, generatedQuestionId) {
    ...
    const conceptId = (0, json_1.readOptionalString)(object.conceptId) ?? fallback.id;
    if (conceptId !== fallback.id) {
        throw new Error(`Question concept mismatch: expected ${fallback.id}, received ${conceptId}.`);
    }
    return {
        id: generatedQuestionId?.trim() || (0, json_1.readString)(object.id, "question.id"),
        conceptId,
        difficulty: enumValue(object.difficulty, "question.difficulty", DIFFICULTIES),
...
function evidenceKeysFromQuestion(question) {
    ...
    const add = (references) => {
        for (const reference of references)
            keys.add(reference.attachmentKey);
    };
    add(question.evidence);
    for (const claim of question.expectedClaims)
        add(claim.evidence);
...
function criterionGrade(value, index, rubricById, normalization) {
    ...
    const criterionId = (0, json_1.readString)(object.criterionId, `criterionGrades[${index}].criterionId`);
    const rubric = rubricById.get(criterionId);
    if (!rubric)
        throw new Error(`Unknown criterion grade: ${criterionId}`);
    return {
        criterionId,
        score: (0, json_1.readNumber)(object.score, `criterionGrades[${index}].score`, {
            min: 0,
            max: rubric.maxScore,
        }),
        // The trusted blueprint owns the maximum; never trust an echoed model value.
        maxScore: rubric.maxScore,
        feedback: (0, json_1.readString)(object.feedback, `criterionGrades[${index}].feedback`),
        evidence: (0, types_1.normalizeEvidenceReferences)(object.evidence, normalization),
    };
}
function parseMasteryGradeResponse(text, question) {
    ...
    const normalization = allowedAttachmentKeys.size > 0 ? { allowedAttachmentKeys } : {};
    const criterionGrades = (0, json_1.readArray)(object.criterionGrades, "grade.criterionGrades").map((entry, index) => criterionGrade(entry, index, rubricById, normalization));
    uniqueIds(criterionGrades.map((grade) => ({ id: grade.criterionId })), "grade.criterionGrades");
    const receivedIds = new Set(criterionGrades.map((criterion) => criterion.criterionId));
    for (const id of rubricById.keys()) {
        if (!receivedIds.has(id))
            throw new Error(`Missing criterion grade: ${id}`);
    }
    const misconceptions = (0, json_1.readArray)(object.misconceptions ?? [], "grade.misconceptions").map((value, index) => {
        const misconception = (0, json_1.readObject)(value, `misconceptions[${index}]`);
        return {
            statement: (0, json_1.readString)(misconception.statement, `misconceptions[${index}].statement`),
            severity: enumValue(misconception.severity, `misconceptions[${index}].severity`, SEVERITIES),
            evidence: (0, types_1.normalizeEvidenceReferences)(misconception.evidence, normalization),
        };
    });
    return {
        criterionGrades,
        misconceptions,
        overallFeedback: (0, json_1.readString)(object.overallFeedback, "grade.overallFeedback"),
        explanation: (0, json_1.readString)(object.explanation, "grade.explanation"),
        graderConfidence: (0, json_1.readNumber)(object.graderConfidence, "grade.graderConfidence", {
            min: 0,
            max: 1,
        }),
    };
