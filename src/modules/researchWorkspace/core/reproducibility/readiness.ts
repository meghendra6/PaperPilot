...
function calculateReproducibilityReadiness(report: {
  ...
  for (const artifact of report.artifacts) {
    ...
    if (artifact.availability === "not_applicable") continue;
    ...
    availability.set(
      artifact.kind,
      Math.max(availability.get(artifact.kind) ?? 0, value),
    );
  }
  for (const group of REQUIRED_GROUPS) {
    if (group.some((kind) => declaredKinds.has(kind))) continue;
  ...
  const majorBlockers = report.blockers.filter(
    (entry) => entry.severity === "major",
  ).length;
  const blockerPenalty =
    criticalBlockers * 0.18 +
    majorBlockers * 0.08 +
    report.blockers.filter((entry) => entry.severity === "minor").length *
      0.025;
  const score = Math.max(
    0,
    Math.min(
      1,
      (totalWeight ? availableWeight / totalWeight : 0) - blockerPenalty,
    ),
  );
  const missingKinds = [...availability.entries()]
    .filter(([, value]) => value < 0.5)
    .map(([kind]) => kind);
  const label =
    criticalBlockers || score < 0.35
      ? "not_ready"
      : score < 0.6
        ? "partial"
        : score < 0.82
...
export { calculateReproducibilityReadiness };
