...
function buildLiteratureGraphPrompt(params) {
  return `Build an evidence-grounded literature graph in ${params.responseLanguage || "English"}. Return JSON only: {nodes:[{id,kind,label,paperKey?,metadata?}],edges:[{id,source,target,kind,label?,confidence,evidence,verified}]}. Allowed kinds: paper,concept,claim,method,dataset. Edge kinds: introduces,uses,extends,improves,challenges,supports,contradicts,compares,evaluates_on,related. Every non-trivial edge must carry evidence from one supplied paper. Every evidence reference must copy sourceID, libraryID, and attachmentKey from that paper. Mark inferred edges verified:false.\n${sourceBlock("papers", JSON.stringify(params.papers))}`;
}
