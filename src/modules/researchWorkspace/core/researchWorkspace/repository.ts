...
class ResearchWorkspaceRepository {
  ...
  async load() {
    ...
    try {
      ...
      throw new Error(
        `Research Workspace contains invalid JSON at ${this.path}: ${String(error)}`,
      );
    }
  ...
  async saveUnlocked(state) {
    ...
    if (!write)
      throw new Error(
        "ResearchWorkspaceStorage requires writeAtomic or writeTextAtomic.",
      );
    await write.call(
      this.storage,
      this.path,
...
export { ResearchWorkspaceRepository };
