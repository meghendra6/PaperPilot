...
function ndcg(returned: string[], relevant: Set<string>) {
  ...
  for (
    let index = 0;
    index < Math.min(returned.length, relevant.size);
    index += 1
  )
...
function evaluateHybridRetrieval(
  ...
  return {
    ...
    recallAtK:
      perCase.reduce(
        (sum, entry, indexValue) =>
          sum +
          entry.relevantFound.length /
            Math.max(1, cases[indexValue].relevantChunkIds.length),
        0,
      ) / denominator,
    precisionAtK:
      perCase.reduce(
        (sum, entry) => sum + entry.relevantFound.length / effectiveK,
        0,
      ) / denominator,
    meanReciprocalRank:
      perCase.reduce((sum, entry) => sum + entry.reciprocalRank, 0) /
      denominator,
    normalizedDiscountedCumulativeGain:
      perCase.reduce((sum, entry) => sum + entry.ndcg, 0) / denominator,
    perCase,
  };
}

export { evaluateHybridRetrieval };
